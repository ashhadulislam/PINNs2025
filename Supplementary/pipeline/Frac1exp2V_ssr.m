...
res=double(py.F_frac1exp2V.fit_frac1exp2V(input_data, alpha, beta));
...
if min([E R a b P0])>1e-10 %Routh-Hurwitz criterion, a>0 represents a positive correlation between P and V. set 1e-10 as 0 to avoid singular point
	...
	input=data(:,1)-P0;
	simin=[t2;input']';
	set_param('model_frac1exp2V','StopTime','length(data)/100');
	TestOut = sim('model_frac1exp2V','ReturnWorkspaceOutputs','on');
	%Evaluation
