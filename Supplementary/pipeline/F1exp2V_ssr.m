function ssr = F1exp2V_ssr(input_parameters)
global input_data
global x
global E
global R
...
[E R a P0]=F1exp2V(input_data,input_parameters(1));
...
if min([E R a P0])>1e-10 %Routh-Hurwitz criterion, a>0 represents a positive correlation between P and V. set 1e-10 as 0 to avoid singular point
	...
	input=data(:,1)-P0;
	simin=[t2;input']';
	set_param('model_1exp2V','StopTime','length(data)/100');
	TestOut = sim('model_1exp2V','ReturnWorkspaceOutputs','on');
	%Evaluation
