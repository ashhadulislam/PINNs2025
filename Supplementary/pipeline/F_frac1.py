...
def fit_frac1(input_file, order):
	global ori_matrix
	ori_matrix = rdnumpy(input_file)#P、V'、V
	new_matrix = np.ones((ori_matrix.shape[0], ori_matrix.shape[1]+1), dtype=float)#
...
#		res=df.GL(order-1, funcSpeed, 0, 149.99, 15000)
	...
	res=df.GL(order, funcV, 0, 149.99, 15000)
#	if (order<1):
#		res=df.RL(order, funcV, 0, 149.99, 15000)
#	elif (order<2):
#		res=df.RL(order-1, funcSpeed, 0, 149.99, 15000)
#	else:
