...
#P=V/C+aV'(alpha)+P0 | P=EV+aV'(alpha)+P0;
def fit_frac0(input_file, order):
	global ori_matrix
	ori_matrix = rdnumpy(input_file)#P、V'、V
	new_matrix = np.ones((ori_matrix.shape[0], ori_matrix.shape[1]), dtype=float)#
	new_matrix[:,0]=ori_matrix[:,2]#V→L
#	new_matrix[:,1]=ori_matrix[:,1]#V‘ →L/s #aV'(alpha)
...
#		res=df.GL(order-2, funcA, 0, 149.99, 15000)
	...
	for tmp_row in range(0, ori_matrix.shape[0]):
		new_matrix[tmp_row,1]=res[tmp_row]
